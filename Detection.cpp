...
int Detection::Ellipse(const char* arg1)
{
    fitEllipseQ = true;
    fitEllipseAMSQ = true;
   ...
   /* cv::CommandLineParser parser(argc, argv, "{help h||}{@image|ZOeuf1T.jpg|}");
    if (parser.has("help"))
    {
        help();
        return 0;
...
void processImage(int /*h*/, void*)
{
   ...
   /*   if (fitEllipseQ) {
        ...
        if (fitEllipseQ && fitEllipseAMSQ && fitEllipseDirectQ) {
            if (box.size.height<= 100 && box.size.width<=100 && box.size.height >= 40 && box.size.width >= 40) {
                cout << "\nOeuf n°:" <<oeuf<< " SIZE_height "<< box.size.height<<endl;
                cout << "Oeuf n°:" << oeuf << " SIZE_width " << box.size.width << endl;
    ...
    imwrite(".\\resultFINAL.jpg", paper.img);
}